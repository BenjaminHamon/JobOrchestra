...
def run(environment, configuration, arguments): # pylint: disable = unused-argument
	...
	temporary_directory = configuration.get("temporary_directory", None)
	...
	install_packages(python_executable, python_package_repository, [ development_toolkit_package ], temporary_directory, simulate = arguments.simulate)
	print("")

	if len(configuration.get("development_dependencies", [])) > 0:
		logger.info("Installing development dependencies")
		install_packages(python_executable, python_package_repository, configuration["development_dependencies"], temporary_directory, simulate = arguments.simulate)
		print("")

	if len(configuration.get("project_dependencies", [])) > 0:
		logger.info("Installing project dependencies")
		install_packages(python_executable, python_package_repository, configuration["project_dependencies"], temporary_directory, simulate = arguments.simulate)
		print("")

	for component in configuration["components"]:
		setup_component(configuration, component, simulate = arguments.simulate)
	print("")
	...
	install_packages(python_executable, python_package_repository, development_package_collection, temporary_directory, simulate = arguments.simulate)


def install_packages(python_executable, python_package_repository, package_collection, temporary_directory, simulate):
	...
	if temporary_directory is not None:
		subprocess_environment["TMPDIR"] = os.path.abspath(temporary_directory)
	...
	if not simulate:
		subprocess.check_call(install_command, env = subprocess_environment)
